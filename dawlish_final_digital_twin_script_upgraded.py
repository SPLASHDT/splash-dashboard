...
def process_block(block_date):
    try:
        ...
        Apply_wind_speed_file = get_wind_file("agl_wind-speed-{}", Met_office_wind_folder, block_date)
        wind_direction_file = get_wind_file("agl_wind-direction-{}", Met_office_wind_folder, block_date)
...
def get_digital_twin_dataset():
    ...
    if block_data is not None:
        ...
        final_DawlishTwin_dataset_tmp = block_data[['datetime', 'Hs', 'Tm', 'shoreWaveDir',
                                                'water_level', 'Wind Speed', 'Wind Direction']].copy()
        final_DawlishTwin_dataset_tmp = final_DawlishTwin_dataset_tmp.rename(columns={
            'datetime': 'time',
            'Hs': 'Hs',
            'Tm': 'Tm',
        ...
        final_DawlishTwin_dataset_tmp['time'] = pd.to_datetime(final_DawlishTwin_dataset_tmp['time'])


    # Step 3: Create our slider adjustment bar

        def on_submit_clicked(button):
            mechnaism_for_adjusting_slider = adjust_features(final_DawlishTwin_dataset_tmp)
